...
export function Ring({ value, label, color, size = 68 }: RingProps): React.ReactElement {
  const stroke = 6
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`${label}: ${Math.round(pct)}%`}>
        ...
        />
        <text x="50%" y="50%" dominantBaseline="central" textAnchor="middle" className="fill-ink font-display text-base font-semibold tnum">
          {Math.round(pct)}
        </text>
      </svg>
